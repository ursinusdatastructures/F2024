...
function outputTable(x, y, D) {
   ...
   for (var i = 0; i < D.length; i++) {
       ...
       if (i == 0) {
           ...
           col.innerHTML = "<h2>"+x[i-1]+"</h2>";
       }
       row.appendChild(col);
       for (var j = 0; j < D[i].length; j++) {
           col = document.createElement("td");
           row.appendChild(col);
           col.innerHTML = "<h3>"+D[i][j]+"</h3>";
       }
   }
}
...
function computeBacktracing(path, x, y){
    ...
    do{
        ...
        if(path[i][j] == 'd'){
            ...
            i --;
            j --;
            let x = currentX.slice(0, -1) + "<b>" + currentX.slice(-1) + "</b>";
            let y = currentY.slice(0, -1) + "<b>" + currentY.slice(-1) + "</b>";
            list_print[n] += x + ", " + y + ": Match " + currentX.slice(-1) + " to " + currentY.slice(-1);
        }else if(path[i][j] == 'u'){
            //now go to the element above
            i --; //one row up
            let x = currentX.slice(0, -1) + "<b>" + currentX.slice(-1) + "</b>";
            list_print[n] += x + ", " + currentY + ": Delete " + currentX.slice(-1);
        }else if(path[i][j] == 'l'){
            //now fo to the element to the left
            j --; //one col over
            let y = currentY.slice(0, -1) + "<b>" + currentY.slice(-1) + "</b>";
            list_print[n] += currentX + ", " + y + ": Delete " + currentY.slice(-1);
        }
        else {
            break;
        }
